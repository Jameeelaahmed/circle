...
export async function addMemberToCircle(circleId, newMemberUid, currentUser) {
    ...
    try {
        ...
        const currentUserMemberRef = doc(db, "circles", circleId, "members", currentUser.uid);
 ...
 */
export async function toggleMemberAdminRole(circleId, targetMemberUid, makeAdmin, currentUser) {
    const db = getFirestore();
    ...
    try {
        ...
        // Get target member
        const targetMemberRef = doc(db, "circles", circleId, "members", targetMemberUid);
        ...
        if (!makeAdmin && targetMemberData.isAdmin) {
            ...
            const adminCount = membersSnapshot.docs.filter(doc => doc.data().isAdmin).length;

            if (adminCount <= 1) {
                return {
                    success: false,
                    message: "Cannot remove the last admin. Please make someone else admin first."
                };
            }
        }
